...
symbol_config_dict = {
    'son1': {
        'symbol_config':
            {
                'XRP-USDT': {'instrument_id': 'XRP-USDT',  # 现货币对，-前是交易币，-后是计价币
                             'strategy_name': 'real_auto_invest_equivalent_value',  # 定投策略
                             'base_invest_val': 1.0,  # 以计价货币计的基础定投额
                             'invested_times': 0,  # 策略开始时的已投资次数，请准确设定，否则会导致意外交易（不涉及投资次数的策略可随意设）
                             'para': [],
                             },  # 策略参数，对于不涉及参数的策略可不设置
                'TRX-USDT': {'instrument_id': 'TRX-USDT',
                             'strategy_name': 'real_auto_invest_equivalent_value',  # 不同币种可以使用不同的策略
                             'base_invest_val': 1.0,
                             'invested_times': 0,
                             'para': [],
                             },
