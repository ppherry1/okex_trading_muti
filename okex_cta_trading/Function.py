...
# MOD 改下单代码,另需要传入account withdraw和position
# 在合约市场下单
...
# 并行下单
def multi_threading_place_order(exchange, symbol_info, symbol_config, symbol_signal, max_try_amount=5):
    """
    :param exchange:
    :param symbol_info:
    :param symbol_config:
    ...
    :param max_try_amount:
    :return:
    并行使用okex_future_place_order()函数，下单
    ...
    if symbol_signal:
        # 准备并行
        f = partial(okex_future_place_order, exchange, symbol_info, symbol_config, symbol_signal, max_try_amount)
        arglist = [symbol for symbol in symbol_signal.keys()]
        ...
        try:
            pool = Pool(processes=len(arglist))
            result_list = pool.map(f, arglist)
            for symbol, order_id_list in result_list:
                for order_id in order_id_list:
                    symbol_order.loc[order_id, 'symbol'] = symbol
                    # 从symbol_info记录下单相关信息
                    symbol_order.loc[order_id, '信号价格'] = symbol_info.loc[symbol, '信号价格']
